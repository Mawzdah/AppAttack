...
# Function to run automated scan (called from menus)
run_automated_scan() {
    while true; do
    ...
    run_nmap "$ip" "$OUTPUT_DIR" "false"
    echo "y" | generate_ai_insights "$nmap_ai_output" "$output_to_file" "$output_file" "nmap"
    run_nikto "$ip" "$port" "$OUTPUT_DIR"
    echo "y" | generate_ai_insights "$nikto_ai_output" "$output_to_file" "$output_file" "nikto"
    run_owasp_zap "$ip" "$port" "$OUTPUT_DIR"
    echo "y" | generate_ai_insights "$zap_ai_output" "$output_to_file" "$output_file" "zap"
    run_wapiti "$ip" "$port" "$OUTPUT_DIR"
    echo "y" | generate_ai_insights "$wapiti_ai_output" "$output_to_file" "$output_file" "wapiti"
